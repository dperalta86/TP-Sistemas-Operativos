...
int wipe_storage_content(const char* mount_point) {
    ...
    if (stat(mount_point, &st) != 0 || !S_ISDIR(st.st_mode)) {
        log_error(g_storage_logger, "El directorio %s no existe o no es un directorio", mount_point);
        return -1;
    }
    ...
    if (system(command) != 0) {
        log_error(g_storage_logger, "No se pudo limpiar el contenido del directorio %s", mount_point);
        return -1;
    }

    log_info(g_storage_logger, "Contenido del directorio %s limpiado (preservando superblock.config)", mount_point);
    return 0;
}
...
int read_superblock(const char* mount_point, int* fs_size, int* block_size) {
    ...
    if (config == NULL) {
        log_error(g_storage_logger, "No se pudo abrir el archivo %s", superblock_path);
        return -1;
    }

    if (!config_has_property(config, "FS_SIZE") || !config_has_property(config, "BLOCK_SIZE")) {
        log_error(g_storage_logger, "El superblock no tiene las propiedades requeridas (FS_SIZE, BLOCK_SIZE)");
    ...
    *block_size = config_get_int_value(config, "BLOCK_SIZE");
    ...
    log_info(g_storage_logger, "Superblock leído: FS_SIZE=%d, BLOCK_SIZE=%d", *fs_size, *block_size);
    return 0;
}
...
int init_bitmap(const char* mount_point, int fs_size, int block_size) {
    ...
    log_info(g_storage_logger, "Bitmap inicializado en %s con %d bloques", bitmap_path, total_blocks);
...
int init_physical_blocks(const char* mount_point, int fs_size, int block_size) {
    ...
    if (mkdir(physical_blocks_dir_path, 0755) != 0) {
        log_error(g_storage_logger, "No se pudo crear el directorio %s", physical_blocks_dir_path);
        return -1;
    }
    ...
    for (int i = 0; i < total_blocks; i++) {
        ...
        if (block_ptr == NULL) {
            log_error(g_storage_logger, "No se pudo crear el archivo de bloque %s", new_block_path);
            free(zero_buffer);
            return -2;
        }
    ...
    log_info(g_storage_logger, "Creados %d bloques físicos en %s", total_blocks, physical_blocks_dir_path);
    return 0;
}
...
int init_files(const char* mount_point) {
    ...
    if (mkdir(target_path, 0755) != 0) {
        log_error(g_storage_logger, "No se pudo crear el directorio %s", target_path);
        return -1;
    }
    ...
    if (link(source_path, target_path) != 0) {
      log_error(g_storage_logger, "No se pudo crear el hard link %s -> %s", source_path, target_path);
      return -5;
    }
...
int init_storage(const char* mount_point) {
    ...
    if (read_superblock(mount_point, &fs_size, &block_size) != 0) return -1;
    if (wipe_storage_content(mount_point) != 0) return -2;
    if (init_bitmap(mount_point, fs_size, block_size) != 0) return -3;
    if (init_blocks_index(mount_point) != 0) return -4;
    if (init_physical_blocks(mount_point, fs_size, block_size) != 0) return -5;
    if (init_files(mount_point) != 0) return -6;
