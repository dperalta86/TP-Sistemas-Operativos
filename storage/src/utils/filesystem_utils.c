...
bool file_dir_exists(const char *file_name, const char *tag) {
  ...
  snprintf(tag_path, sizeof(tag_path), "%s/files/%s/%s", g_storage_config->mount_point, file_name, tag);
...
t_package *prepare_error_response(uint32_t query_id, t_storage_op_code op_code, int op_error_code) {
  ...
  if(!response)
  {
    log_error(g_storage_logger, "## Query ID: %d - Fallo al crear paquete de respuesta de error", query_id);
    return NULL;
  }
...
bool physical_block_exists(uint32_t block_number, char *physical_block_path, size_t path_size) {
  snprintf(physical_block_path, path_size, "%s/physical_blocks/block%04d.dat", g_storage_config->mount_point, block_number);
...
FILE *open_bitmap_file(const char *modes) {
  char bitmap_path[PATH_MAX];
  snprintf(bitmap_path, sizeof(bitmap_path), "%s/bitmap.bin", g_storage_config->mount_point);

  FILE *bitmap_file = fopen(bitmap_path, modes);
  if (!bitmap_file) {
    log_error(g_storage_logger, "No se pudo abrir el archivo bitmap: %s", bitmap_path);
    return NULL;
  }
...
int bitmap_load(t_bitarray **bitmap, char **bitmap_buffer) {
  ...
  *bitmap_buffer = calloc(1, bitmap_size_bytes);
  ...
  if (fread(bitmap_buffer, 1, bitmap_size_bytes, bitmap_file) != bitmap_size_bytes) {
    log_error(g_storage_logger, "No se pudo leer el bitmap completo");
    pthread_mutex_unlock(&g_storage_bitmap_mutex);
    free(bitmap_buffer);
    retval = -3;
...
void set_bitmap_bits(t_bitarray *bitmap, int start_index, size_t count, int set_bits) {
  ...
  log_info(g_storage_logger, "Modificados %zu bits en el bitmap buffer (%s)", count, set_bits ? "seteados" : "unseteados");
}
